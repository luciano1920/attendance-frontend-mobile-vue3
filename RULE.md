...
- **Vue 组件**：大驼峰 PascalCase，必须以 `.vue` 结尾。全局通用组件（`src/components` 目录下）以 `App` 为前缀（如 `AppBackListTop.vue`, `AppSegmented.vue`）。业务组件按照业务场景和功能命名。
...
- 在 Vue 模板中，TDesign 组件使用短横线命名（如 `<t-list></t-list>`），自定义组件（包括全局和业务模块）使用大驼峰命名（如 `<AppBackListTop></AppBackListTop>`）。
- **正例**：
  - TDesign 组件：`<t-list></t-list>`
  - 自定义组件：`<AppBackListTop></AppBackListTop>`（对应 `App-BackListTop.vue`）
...
- **相等比较**：**严禁使用 `==`**，必须使用 `===`（ESLint 已开启 `vue/eqeqeq: 'error'`）。
